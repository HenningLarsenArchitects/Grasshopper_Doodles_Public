...
/// <summary>
/// MANI - working on a grid that can do iso curves and other stuff.. To be open sourced at one point.
/// </summary>
...
namespace Grasshopper_Doodles_Public
{
    public class Grid
    {
        ...
        public List<double> GetResults(int ID)
        {
            if (ID < Resultss.Count)
                return Resultss[ID].Results.ToList();
            else
                throw new Exception($"Tried to query GetResults with id = {ID}. MaxCount was {Resultss.Count}");
        }
        ...
        {
            if (ID >= Resultss.Count)
                throw new Exception($"Tried to Get name of id = {ID}. MaxCount was {Resultss.Count}");
            return Resultss[ID].Name;
        }

        public void SetResults(string name, IEnumerable<double> values)
        {
            if (values.Count() != SimPoints.Count)
                throw new Exception("lengths of points and data dont match");
            Resultss.Add(new GridResults(name, values));
        }
        ...
        {
            double verticalOffset = Units.ConvertFromMeter(0.5);
